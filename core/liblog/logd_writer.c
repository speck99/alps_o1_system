    ...
    if (sock < 0) {
...
#if defined(MTK_LOGD_ENHANCE) && defined(CONFIG_MT_DEBUG_BUILD)
      /*
      *  set socket Timtout option for BLOCK mode. And
      *  filter out  process 'android.hardware.configstore@1.0-service' which
      ...
      */
      ...
      if (threadname) {
        ...
        if (strstr(threadname, key_configstore) != NULL)
          skip_thread = 1;
      }
