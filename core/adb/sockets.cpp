    ...
    */
    ...
    if (ev & FDE_READ) {
        ...
        if (difftime(curr_time, read_time) > 20.0) {
            local_socket_list_len = 0;
            std::lock_guard<std::recursive_mutex> lock(local_socket_list_lock);
            for (tmp = local_socket_list.next; tmp != &local_socket_list; tmp = tmp->next) {
                local_socket_list_len++;
            }
            ADBLOGDBG("+read_data=%d local_socket_list_len=%d\n", read_data, local_socket_list_len);
            read_time = curr_time;
            read_data = 0;
        }
#endif
