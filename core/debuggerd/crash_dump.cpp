...
int main(int argc, char** argv) {
  ...
  if (signo == SIGTRAP && (siginfo.si_code & 0xff) == SIGTRAP &&
    ((siginfo.si_code >> 8) & 0xff) == PTRACE_EVENT_STOP) {
    for (const auto& it : threads) {
      pid_t thread_tid = it.first;
      if (ptrace(PTRACE_DETACH, thread_tid, 0, 0) != 0) {
        LOG(ERROR) << "failed to detach from thread " << thread_tid;
      }
    }

    // send SIGCONT for dump backtrace/tombstone
    if (kill(target, SIGCONT) != 0) {
      LOG(ERROR) << "failed to resume process " << target;
    }

    LOG(INFO) << "exit for SIGTRAP and PTRACE_EVENT_STOP";
    exit(1);
  }
