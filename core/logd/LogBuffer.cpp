...
#if defined(MTK_LOGD_ENHANCE) && defined(MTK_LOGD_UXFREE_DEBUG)
...
#if defined(MTK_LOGD_ENHANCE) && defined(MTK_LOGD_UXFREE_DEBUG)
    readerBegin = readerLast = mLogElements.begin();
    mWaterMark = log_time(log_time::tv_sec_max, log_time::tv_nsec_max);
#endif
...
#if defined(MTK_LOGD_ENHANCE) && defined(MTK_LOGD_UXFREE_DEBUG)
        readerLast = it;
        if (element->getLogId() > LOG_ID_MAX) {
            android::prdebug("reader.per, readerBegin: %p, readerLast: %p", &readerBegin, &readerLast);
        }
#endif
