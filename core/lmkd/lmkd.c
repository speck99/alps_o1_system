...
static int medium_oomadj, orig_medium_oomadj;
...
/*
 * Loosen medium_oomadj by request.
 * Only requests with higher or equal oomadj levels are allowed.
 * level == 0, change to use default medium_oomadj
 * level >= orig_medium_oomadj, assign level to medium_oomadj
 * level < orig_medium_oomadj, forbidden
 */
static void cmd_medloosen(int level) {
    if (use_inkernel_interface)
        return;

    /* use default medium_oomadj */
    if (level == 0) {
	medium_oomadj = orig_medium_oomadj;
	return;
    }

    /* forbidden */
    if (level < orig_medium_oomadj)
	return;

    /* assign it a equal or higher value */
    medium_oomadj = level;
}
...
static void mp_event_common(bool is_critical) {
    ...
    /* Got a critical vmpressure, restore medium_oomadj */
    if (is_critical) {
	medium_oomadj = orig_medium_oomadj;
    }
}
