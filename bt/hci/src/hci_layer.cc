...
// M: Add a mutex to protect TX race condition with RX thread
static std::mutex command_entry_mutex;
// M: Add a mutex to protect RX alarm_cancel race condition with HCI shutdown
static std::mutex alarm_shutdown_mutex;
...
static void cancel_command_response_timer(void);
...
static future_t* hci_module_shut_down() {
  ...
  // Free the timers
  {
    std::lock_guard<std::mutex> lock(alarm_shutdown_mutex);
    alarm_free(command_response_timer);
    command_response_timer = NULL;
    alarm_free(startup_timer);
    startup_timer = NULL;
    LOG_INFO(LOG_TAG, "%s timers freed with mutex.", __func__);
  }
  ...
  LOG_INFO(LOG_TAG, "%s message loop task stopped.", __func__);
...
static void event_command_ready(waiting_command_t* wait_entry) {
  std::lock_guard<std::mutex> command_entry_lock(command_entry_mutex);
  {
