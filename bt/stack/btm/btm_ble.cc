...
uint8_t btm_proc_smp_cback(tSMP_EVT event, const RawAddress& bd_addr,
  ...
  if (p_dev_rec != NULL) {
    switch (event) {
      ...
      case SMP_COMPLT_EVT:
        /** M: bug fix if SMP_COMPLT_EVT fail, callback at last @{ */
        if ((btm_cb.api.p_le_callback) &&
            ((event != SMP_COMPLT_EVT) || (p_data->cmplt.reason == SMP_SUCCESS))) {
        /** @} */
        ...
        if (event == SMP_COMPLT_EVT) {
          ...
          /** M: bug fix if SMP_COMPLT_EVT fail, callback at last @{ */
          if ((btm_cb.api.p_le_callback) && (p_data->cmplt.reason != SMP_SUCCESS)) {
            BTM_TRACE_DEBUG("btm_cb.api.p_le_callback=0x%x",
                            btm_cb.api.p_le_callback);
