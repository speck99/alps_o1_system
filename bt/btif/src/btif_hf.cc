...
static void btif_hf_upstreams_evt(uint16_t event, char* p_param) {
  /** M: Change for bug fix: Hfp access memory that not belonged to it. @{ */
  if (event == BTA_AG_ENABLE_EVT || event == BTA_AG_DISABLE_EVT)
  {
    BTIF_TRACE_DEBUG("%s: event=%s, return directly to avoid exception", __func__,
                     dump_hf_event(event));
    return;
  }
  /** @} */
  ...
  switch (event) {
...
#if defined(MTK_INTEROP_EXTENSION) && (MTK_INTEROP_EXTENSION == TRUE)
      if (interop_mtk_match_addr_name(INTEROP_MTK_ADVANCED_A2DP_CONNECT,
            &(btif_hf_cb[idx].connected_bda))) {
        btif_queue_advance();
      } else {
        if (p_data->open.status != BTA_AG_SUCCESS) {
    ...
    case BTA_AG_AT_CLCC_EVT:
      /** M: Add for some devices send clcc before slc connected. @{ */
      {
        bool connected = false;
        RawAddress* bd_addr = &btif_hf_cb[idx].connected_bda;
        ...
        if (connected)
          ...
          send_at_result(BTA_AG_OK_DONE, BTA_AG_ERR_OP_NOT_ALLOWED, idx);
      }
      // } @end
      break;
