...
#endif
#include "mediatek/btif/include/mtk_btif_dm.h"
...
/* Vanzo:tanglei on: Thu, 07 Dec 2017 15:46:21 +0800
 * modify for custom bt name
#define PROPERTY_PRODUCT_MODEL "ro.product.model"
 */
#define PROPERTY_PRODUCT_MODEL "ro.product.bt_name"
...
static void bond_state_changed(bt_status_t status, const RawAddress& bd_addr,
  ...
  /** M: If service discovery is in progress, postpone reporting bond state changed @{ */
  if ((state == BT_BOND_STATE_NONE) && (pairing_cb.sdp_attempts !=0) &&
      (bd_addr == pairing_cb.bd_addr)) {
    BTIF_TRACE_DEBUG("%s: postpone this bond state change report", __func__);
  } else {
    auto tmp = bd_addr;
    HAL_CBACK(bt_hal_cbacks, bond_state_changed_cb, status, &tmp, state);
  }
  ...
  /** M: fix unexcepted change of pairing address @{ */
...
static void btif_dm_pin_req_evt(tBTA_DM_PIN_REQ* p_pin_req) {
  ...
  if (pairing_cb.is_local_initiated &&
      (p_pin_req->bd_addr != pairing_cb.bd_addr)) {
    BTIF_TRACE_WARNING("%s local initiated pairing to another"
                       "device haven't finished", __func__);
...
/** M: Add for ble device pair on guest mode  @{ */
  ...
  {
      const char* bdstr = bd_addr.ToString().c_str();
...
/* Vanzo:tanglei on: Thu, 07 Dec 2017 15:45:01 +0800
 * modify for custom bt name
