...
static RawAddress mtk_rc_peer_addr;
/** @} */
/*****************************************************************************
 *  Constants & Macros
 *****************************************************************************/
...
static void btif_initiate_av_open_timer_timeout(UNUSED_ATTR void* data) {
  ...
  memset(&mtk_rc_peer_addr, 0, sizeof(RawAddress));
  ...
  /** @} */
    ...
    connect_req.target_bda = &mtk_rc_peer_addr;
    /** @} */
...
/** M: Change for bug fix: Do not close avrcp after sdp of audio sink failed. @{ */
            ...
            if (!(p_bta_data->open.status == BTA_AV_FAIL_SDP) &&
                interop_mtk_match_addr_name(
                    INTEROP_MTK_AUDIO_SINK_SDP_FAIL_NOT_DISC_AVRCP, &peer_addr))
#endif
/** @} */
      ...
      if (p_bta_data->open.status == BTA_AV_SUCCESS &&
      /** @} */
          btif_av_cb.self_initiated_connection) {
        btif_queue_advance();
      }
    } break;
  ...
  switch (event) {
    case BTIF_SM_ENTER_EVT:
      ...
      if (interop_mtk_match_addr_name(
                    INTEROP_MTK_SKIP_REMOTE_SUSPEND_FLAG, &(btif_av_cb.peer_bda))) {
        BTIF_TRACE_EVENT("%s: skip flag  BTIF_AV_FLAG_REMOTE_SUSPEND", __FUNCTION__);
        btif_av_cb.flags &= ~BTIF_AV_FLAG_REMOTE_SUSPEND;
      }
#endif
      /** @} */
    ...
    case BTA_AV_START_EVT: {
      ...
      if (p_av->start.status != BTA_AV_SUCCESS) {
        ...
        BTA_AvDisconnect(btif_av_cb.peer_bda);
        /** @} */
        return false;
      }
