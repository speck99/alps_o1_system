...
#include "mediatek/btif/include/mtk_btif_rc.h"

#if defined(MTK_AVRCP_APP_SETTINGS) && (MTK_AVRCP_APP_SETTINGS == TRUE)
using vendor::mediatek::bluetooth::btif::BtAvrcSetting;
#endif
using vendor::mediatek::bluetooth::btif::BtAvrcStatusHandling;
static bt_status_t register_notification_rsp(
    btrc_event_id_t event_id, btrc_notification_type_t type,
    btrc_register_notification_t* p_param);
#endif

/** M: Check if SCO exist. @{ */
#include "btif_hf.h"
/** @} */
...
#if defined(MTK_INTEROP_EXTENSION) && (MTK_INTEROP_EXTENSION == TRUE)
      interop_mtk_match_addr_name(INTEROP_MTK_DISABLE_ABSOLUTE_VOLUME,
                                  &rc_addr) ||
#endif
...
// Some IOT device will have wrong song position when FF/REW
  ...
  if (interop_mtk_match_addr(INTEROP_MTK_AVRCP_DISABLE_SONG_POS, &p_dev->rc_addr)
        && interop_mtk_match_name(INTEROP_MTK_AVRCP_DISABLE_SONG_POS, &p_dev->rc_addr))
  {
...
void handle_rc_passthrough_cmd(tBTA_AV_REMOTE_CMD* p_remote_cmd) {
  ...
  if (btif_av_is_idle())
  {
    APPL_TRACE_WARNING("%s: AVDT not connection, drop command", __func__);
    return;
  }
  /** @} */

  /** M: Ignore play while SCO exist. @{ */
  // Some carkit sends the PLAY during SCO exist will cause
  // Wechat always in playing status.
  if ((p_remote_cmd->rc_id == BTA_AV_RC_PLAY) && (!btif_hf_is_call_idle())) {
    APPL_TRACE_WARNING("%s: +++SCO Exist, ignore avrcp PLAY command", __func__);
    return;
  }
  /** @} */
  ...
  /** M: Ignore play while sco disc. @{ */
#if defined(MTK_BT_PROPRIETARY_HANDLING) && (MTK_BT_PROPRIETARY_HANDLING == TRUE)
  // After sync with Customer, ignore the first PLAY key during some time(3s).
  // If the first Event is not PLAY event, don't ignore; there is a tricker we
  // change the sco_disc_time to avoid the following PLAY event ignore.
  if ((p_remote_cmd->rc_id != BTA_AV_RC_PLAY) &&
        (time(NULL) - sco_disc_time <= 3)) {
    APPL_TRACE_WARNING("%s: reset sco_disc_time", __func__);
    sco_disc_time = 1;
  }
  if (BtAvrcStatusHandling::IsNeededIgnorePlay(sco_disc_time, p_remote_cmd,
...
void handle_rc_metamsg_cmd(tBTA_AV_META_MSG* pmeta_msg) {
  ...
  /** M: Reject browsing command if remote device does not support browsing. @{ */
  if (!(p_dev->rc_features & BTA_AV_FEAT_BROWSE) &&
    (pmeta_msg->p_msg->hdr.opcode == AVRC_OP_BROWSE))
...
static void send_metamsg_rsp(btif_rc_device_cb_t* p_dev, int index,
  ...
  if ((pmetamsg_resp->rsp.pdu == AVRC_PDU_REGISTER_NOTIFICATION) &&
    ...
    if (bNotify
#if defined(MTK_BT_PROPRIETARY_HANDLING) && (MTK_BT_PROPRIETARY_HANDLING == TRUE)
         || BtAvrcStatusHandling::GetForceUpdateFlag()
#endif
    ) {
    /** @} */
      BT_HDR* p_msg = NULL;
...
#if defined(MTK_AVRCP_APP_SETTINGS) && (MTK_AVRCP_APP_SETTINGS == TRUE)
      BtAvrcSetting::GetInstance()->BtifRcUpstreamsSettingEvt(bt_rc_callbacks,
              event, pavrc_cmd, ctype, label, p_dev, &rc_addr, p_dev->rc_handle);
#else
...
#if defined(MTK_AVRCP_APP_SETTINGS) && (MTK_AVRCP_APP_SETTINGS == TRUE)
  BtAvrcSetting::SetAvrcSettingCB(
    [] (int index, uint8_t ctype, uint8_t label, bool pending, void* p_dev) {
      fill_pdu_queue(index, ctype, label, true, (btif_rc_device_cb_t*)p_dev);
    }, send_reject_response, opcode_from_pdu,
    [](const RawAddress* bd_addr) {
      CHECK_RC_CONNECTED(btif_rc_get_device_by_bda(bd_addr));
      return BT_STATUS_SUCCESS;
    },
    [](const RawAddress* bd_addr, int index, tAVRC_RESPONSE* pmetamsg_resp) {
      btif_rc_device_cb_t* p_dev = btif_rc_get_device_by_bda(bd_addr);
      send_metamsg_rsp(p_dev, index, p_dev->rc_pdu_info[index].label,
          p_dev->rc_pdu_info[index].ctype, pmetamsg_resp);
    });
#endif
  for (int idx = 0; idx < BTIF_RC_NUM_CONN; idx++) {
...
/** @} */
  ...
  /** M: Ignore play while sco disc @{ */
#if defined(MTK_BT_PROPRIETARY_HANDLING) && (MTK_BT_PROPRIETARY_HANDLING == TRUE)
  if (BtAvrcStatusHandling::GetIgnoreAvrcpStatusFlag() &&
        play_status == PLAY_STATUS_PLAYING) {
    BtAvrcStatusHandling::CancelTimer();
    BTIF_TRACE_WARNING("%s: cancel the timer and update the real status.", __func__);
  }
#endif
  /** @} */
...
static bt_status_t register_notification_rsp(
  ...
  for (int idx = 0; idx < BTIF_RC_NUM_CONN; idx++) {
    ...
    /** M: Ignore play while sco disc. @{ */
    // Some time, remote device will not register the status change
    // event, in order to send the fake play/pause status, ignore the
    // bNotify.
    if (
#if defined(MTK_BT_PROPRIETARY_HANDLING) && (MTK_BT_PROPRIETARY_HANDLING == TRUE)
        !BtAvrcStatusHandling::GetForceUpdateFlag() &&
#endif
    /** @} */
          btif_rc_cb.rc_multi_cb[idx].rc_notif[event_id - 1].bNotify == false) {
      BTIF_TRACE_WARNING(
    ...
    /** M: Ignore play while sco disc. @{ */
    // During waiting for the fake play/pause status, ignore the play status
    // update to avoid UI refreshing.
#if defined(MTK_BT_PROPRIETARY_HANDLING) && (MTK_BT_PROPRIETARY_HANDLING == TRUE)
    if (BtAvrcStatusHandling::GetIgnoreAvrcpStatusFlag() &&
            (event_id == BTRC_EVT_PLAY_STATUS_CHANGED)) {
        // The first remote play is ignored, we assume this play is triggered
        // by SRC.
        if (p_param->play_status == PLAY_STATUS_PLAYING) {
            BtAvrcStatusHandling::CancelTimer();
            BTIF_TRACE_WARNING("Avrcp Event id: cancel the timer and update the real status.");
        } else {
            BTIF_TRACE_WARNING("Avrcp Event id: ignore the update.");
            return BT_STATUS_SUCCESS;
        }
    }
#endif
    ...
    switch (event_id) {
      ...
      case BTRC_EVT_APP_SETTINGS_CHANGED:
        BtAvrcSetting::GetInstance()->SetAppSettingsChange(&avrc_rsp.reg_notif.param.player_setting,
            &p_param->player_setting);
        break;
#endif
...
static const btrc_interface_t bt_rc_interface = {
    ...
    get_play_status_rsp,
#if defined(MTK_AVRCP_APP_SETTINGS) && (MTK_AVRCP_APP_SETTINGS == TRUE)
    BtAvrcSetting::ListPlayerAppAttrRsp,
    BtAvrcSetting::ListPlayerAppValueRsp,
    BtAvrcSetting::GetPlayerAppValueRsp,
    BtAvrcSetting::GetPlayerAppAttrTextRsp,
    BtAvrcSetting::GetPlayerAppValueTextRsp,
    get_element_attr_rsp,
    BtAvrcSetting::SetPlayerAppValueRsp,
#else
