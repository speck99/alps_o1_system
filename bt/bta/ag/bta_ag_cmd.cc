...
#include "mediatek/include/mtk_bta_ag_cmd.h"
#endif
/*****************************************************************************
 *  Constants
 ****************************************************************************/
  ...
  switch (cmd) {
    ...
    case BTA_AG_AT_CBC_EVT:
      ...
      if ((cmd == BTA_AG_SPK_EVT ||cmd == BTA_AG_MIC_EVT) && val.num == 0) {
        ...
        APPL_TRACE_DEBUG("%s: cur_time = %ld, hfp_conn_time = %ld", __func__, cur_time, hfp_conn_time);
...
void bta_ag_hfp_result(tBTA_AG_SCB* p_scb, tBTA_AG_API_RESULT* p_result) {
 ...
 #if defined(MTK_INTEROP_EXTENSION) && (MTK_INTEROP_EXTENSION == TRUE)
      /** M: bug fix for end call display on carkit. 600ms is ok test result for the carkit @{ */
