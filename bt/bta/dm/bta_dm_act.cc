...
#endif
#include "mediatek/bta/include/mtk_bta_dm.h"
...
static void bta_dm_discover_device(const RawAddress& remote_bd_addr) {
  ...
  if (bta_dm_search_cb.services) {
    ...
    if (bta_dm_search_cb.services_to_search) {
      ...
      /** M: bug fix, only check BR transport @{ */
...
#if defined(MTK_INTEROP_EXTENSION) && (MTK_INTEROP_EXTENSION == TRUE)
        (!interop_mtk_match_addr_name(INTEROP_MTK_HID_NOT_DO_SNIFF_SUBRATING, &p_bda)) &&
#endif
...
static void bta_dm_remove_sec_dev_entry(const RawAddress& remote_bd_addr) {
  if (BTM_IsAclConnectionUp(remote_bd_addr, BT_TRANSPORT_LE) ||
    ...
    /** M: set SM4 to UNKNOWN after link is down @{*/
