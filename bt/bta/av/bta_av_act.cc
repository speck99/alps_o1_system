...
#include "mediatek/bta/include/mtk_bta_av_act.h"
using vendor::mediatek::bluetooth::bta::BtaAvAct;
#endif
...
/** M: Bug fix for avrcp version dynamic adjust. @{ */
#if defined(MTK_BT_PROPRIETARY_HANDLING) && (MTK_BT_PROPRIETARY_HANDLING == TRUE)
              if (false == BtaAvAct::BtaUpdateAvrcCapRsp(p_rc_rsp))
#endif
/** @} */
              {
...
/** M: Bug fix for browsing channel not setup. @{ */
#if defined(MTK_BT_PROPRIETARY_HANDLING) && (MTK_BT_PROPRIETARY_HANDLING == TRUE)
      BtaAvAct::BtaCleanBrowsingTimer();
#endif
/** @} */
      ...
      for (xx = 0; xx < BTA_AV_NUM_LINKS; xx++) {
        ...
        if ((!(mask & p_cb->conn_lcb)) && (p_cb->p_scb[xx] != NULL)) {
          ...
          if (p_data->hdr.offset == AVDT_ACP) {
            ...
            if (interop_mtk_match_addr_name(INTEROP_MTK_A2DP_CHANGE_ACCEPT_SIGNALLING_TMS, (const RawAddress *)&p_cb->p_scb[xx]->peer_addr))
            {
                ms_t = 6000;
            }
  ...
  while (true) {
    ...
    if ((SDP_FindAttributeInRec(p_rec, ATTR_ID_BT_PROFILE_DESC_LIST)) != NULL) {
      ...
      if (UUID_SERVCLASS_AV_REMOTE_CONTROL == service_uuid)
          BtaAvAct::UpdateAvrcpVersion(get_peer_rc_version, peer_rc_version, p_cb);
#endif
      /** @} */
    }
  }
...
/** M: Bug fix for browsing channel not setup. @{ */
#if defined(MTK_BT_PROPRIETARY_HANDLING) && (MTK_BT_PROPRIETARY_HANDLING == TRUE)
    BtaAvAct::BtaCreateBrowsingTimer(((p_cb->features & BTA_AV_FEAT_BROWSE) &&
        (peer_features & BTA_AV_FEAT_BROWSE)), rc_handle);
#endif
/** @} */
  }
}
 ...
 *
 ******************************************************************************/
void bta_av_rc_disc_ar(uint8_t disc, bool get_rc_version) {
