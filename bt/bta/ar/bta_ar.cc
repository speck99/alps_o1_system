 ...
 *
 *****************************************************************************/
void bta_ar_update_avrc_version(uint16_t peer_rc_version, tBTA_SYS_ID sys_id, uint16_t categories) {
  uint8_t mask = bta_ar_id(sys_id);

  if (mask) {
     BtaAvAct::UpdateSdpAvrcVersion(bta_ar_cb.sdp_tg_handle, peer_rc_version, categories);
  }
}
/** @} */
#endif